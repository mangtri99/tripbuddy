  ...
  try {
    ...
    const usersData = [
      ...
      },
      {
        ...
        name: "Emma Davis",
        bio: "Beach enthusiast and sunset chaser",
      },
    ...
    for (const dest of DESTINATIONS) {
      const placesForCity = Array.from(
    ...
    for (const user of createdUsers.slice(0, 8)) {
      ...
      for (let i = 0; i < numTrips; i++) {
        ...
        endDate.setDate(
          endDate.getDate() + faker.number.int({ min: 3, max: 14 }),
        );
    ...
    for (const user of createdUsers.slice(0, 10)) {
      const placesToSave = randomItems(
        createdPlaces,
      ...
      for (const place of placesToSave) {
        await db.insert(schema.savedPlaces).values({
          ...
          userId: user.id,
          placeId: place.id,
          listName: faker.helpers.arrayElement([
            "Favorites",
            "Want to Visit",
            "Bucket List",
            "Restaurants",
    ...
    console.log(
      `   ✓ Created ${sharedExpenseCount} shared expenses with participants`,
    );
    ...
    for (const user of createdUsers.slice(0, 12)) {
      await db.insert(schema.travelPartnerProfiles).values({
        ...
        countriesVisited: randomItems(
          DESTINATIONS.map((d) => d.country),
          faker.number.int({ min: 2, max: 8 }),
        ),
        preferredDestinations: randomItems(
        ...
        isVerified: faker.datatype.boolean(),
        responseRate: faker.number.float({
          min: 0.5,
          max: 1,
          fractionDigits: 2,
        }),
    ...
    for (const user of createdUsers.slice(0, 8)) {
      ...
      const endDate = new Date(startDate);
      endDate.setDate(
        endDate.getDate() + faker.number.int({ min: 5, max: 14 }),
      );

      const [search] = await db
        .insert(schema.partnerSearches)
        .values({
          id: createId(),
          userId: user.id,
          destination: `${dest.city}, ${dest.country}`,
    ...
    for (const search of createdSearches.slice(0, 5)) {
      const potentialPartners = createdUsers.filter(
        (u) => u.id !== search.userId,
      );
